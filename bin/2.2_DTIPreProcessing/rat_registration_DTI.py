...
def regABA2DTI(inputVolume,stroke_mask,refStroke_mask,T2data, brain_template,brain_anno, splitAnno,splitAnno_rsfMRI,anno_rsfMRI,bsplineMatrix,outfile):
    ...
    outputAnnoparScaled = os.path.join(outfileDSI, os.path.basename(inputVolume).split('.')[
        0] + 'AnnoSplit_parental_scaled.nii')  #> removed '.gz' ending to correct atlas implementation // VVF 23/05/10
    outputSigmaBScaled = os.path.join(outfileDSI, os.path.basename(inputVolume).split('.')[0] + 'Sigma_scaled.nii') #> removed '.gz' ending to correct atlas implementation // VVF 23/05/10
    ...
    dataAnnopar = nii.load(os.path.join(outfile, os.path.basename(inputVolume).split('.')[0] + '_AnnoSplit_parental.nii.gz'))
    dataSigma = nii.load(os.path.join(outfile, os.path.basename(inputVolume).split('.')[0] + '_Template.nii.gz'))

    imgTempAnno = dataAnno.get_fdata()
    imgTempAnnopar= dataAnnopar.get_fdata()
    imgTempSigma = dataSigma.get_fdata()

    imgTempSigma = np.flip(imgTempSigma, 2)
    imgTempAnno = np.flip(imgTempAnno, 2)
    imgTempAnnorsfMRI = np.flip(imgTempAnnopar, 2)
    scale = np.eye(4) * 10
    scale[3][3] = 1

    unscaledNiiDataAnno = nii.Nifti1Image(imgTempAnno, dataAnno.affine * scale)
    unscaledNiiDataAnnopar = nii.Nifti1Image(imgTempAnnopar, dataAnnopar.affine * scale)
    unscaledNiiDataSigma = nii.Nifti1Image(imgTempSigma, dataSigma.affine * scale)
    hdrOut = unscaledNiiDataAnno.header
    hdrOut.set_xyzt_units('mm')
    hdrOut = unscaledNiiDataAnnopar.header
    hdrOut.set_xyzt_units('mm')
    hdrOut = unscaledNiiDataSigma.header
    hdrOut.set_xyzt_units('mm')
    nii.save(unscaledNiiDataAnno, outputAnnoScaled)
    nii.save(unscaledNiiDataAnnopar, outputAnnoparScaled)
    nii.save(unscaledNiiDataSigma, outputSigmaBScaled)
