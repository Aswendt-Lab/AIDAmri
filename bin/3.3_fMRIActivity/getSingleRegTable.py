...
def getRegrTable(file_name,physio_Folder,parPath_folder):
    ...
    if not os.path.exists(par_Path):
        sys.exit("Error: %s is not an existing directory of file." % (par_Path,))
    ...
    if not os.path.exists(i32_Path):
        sys.exit("Error: %s is not an existing directory of file." % (i32_Path,))
...
if __name__ == "__main__":
    ...
    requiredNamed.add_argument('-p', '--physio_Folder', help='file name of data', required=True)
    ...
    if not os.path.exists(physio_Folder):
        sys.exit("Error: '%s' is not an existing directory of file %s is not in directory." % (physio_Folder, args.file,))
