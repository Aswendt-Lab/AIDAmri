...
if __name__ == "__main__":
    ...
    requiredNamed.add_argument('-i', '--inputVolume', help='file name of data', required=True)

    parser.add_argument('-s', '--deformationStrength', help='integer: 1 - very strong defomation, 2 - strong deformation, 3 - medium deformation, 4 - weak deformation ', nargs='?', type=int,
                        default=3)
    ...
    parser.add_argument('-f', '--allenBrain_annorsfMRI', help='file name:Annotations of Allen Brain', nargs='?',
                        type=str,
                        default=os.path.abspath(
                            os.path.join(os.getcwd(), os.pardir, os.pardir)) + '/lib/annoVolume.nii.gz')
    ...
    if len(stroke_mask) is 0:
        stroke_mask = []
        print("Notice: '%s' has no defined stroke mask." % (inputVolume,))
    else:
