...
def calculate_parental_stroke_overlap(brain_file, parental_annotation_file, Ref_template_file, stroke_mask_file,
    ...
    # Load label IDs and names from the parental atlas label table.
    all_label_ids, label_names_by_id, labelNames = load_label_table(label_file)
    parental_label_ids = all_label_ids.copy()

    # Save the template-space IncidenceData lesion mask labelled by the parental atlas.
    ...
    ensure_same_grid(incidence_lesion_mask_img, Ref_template_img, "parental incidence atlas")
    parental_incidence_atlas = np.round(Ref_template_labels)
    ...
    label_mat = {}
    label_mat['SIGLabelsIDsParental'] = parental_label_ids
    label_mat['SIGNamesPar'] = affected_label_names
...
if __name__ == "__main__":
    ...
    parser.add_argument('-a', '--RefBrain_anno', help='File: reference parental atlas annotation template', nargs='?', type=str,
                        default=os.path.join(REPO_ROOT, 'lib', 'sigma', 'SIGMA_InVivo_Anatomical_Brain_Atlas.nii.gz'))

    input_folder = None
    Ref_template_file = None
    output_folder = None
    ...
    if args.RefBrain_anno is not None:
        Ref_template_file = args.RefBrain_anno
    if not os.path.isfile(Ref_template_file):
        sys.exit("Error: '%s' is not an existing file." % (Ref_template_file,))
    ...
    calculate_parental_stroke_overlap(brain_file, parental_annotation_file, Ref_template_file, stroke_mask_file,
                                      incidence_lesion_mask_file, output_folder, label_file)
