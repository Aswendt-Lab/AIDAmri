...
def calculate_stroke_overlap(brain_file, annotation_file, ara_template_file, stroke_mask_file,
    ...
    # Load subject-space annotation and brain image.
    annotation_img = nii.load(annotation_file)
    annotation = np.round(annotation_img.get_fdata())
    brain_img = nii.load(brain_file)
    brain_volume = brain_img.get_fdata()

    ensure_same_grid(stroke_mask_img, annotation_img, "annotation")
    ensure_same_grid(stroke_mask_img, brain_img, "brain image")

    # Keep only atlas labels that overlap with the subject-space stroke mask.
    labelled_stroke_overlap = annotation * stroke_mask

    # Extract the unique non-zero labels affected by the stroke.
    affected_labels = np.unique(labelled_stroke_overlap)
    affected_labels = affected_labels[affected_labels > 0.0]

    # Calculate how much of each affected region is covered by the stroke.
    region_percent_by_label = {}
    region_affected_voxels_by_label = {}
    for label_id in affected_labels:
        # Percentage of the atlas region covered by stroke voxels.
        affected_voxels = np.sum(labelled_stroke_overlap == label_id)
        total_region_voxels = np.sum(annotation == label_id)
    ...
    affected_label_mask = np.isin(atlas_label_ids, affected_label_ids)
    atlas_label_ids = atlas_label_ids[affected_label_mask]
    ...
    output_file = os.path.join(affected_regions_dir, affected_regions_prefix + 'affectedRegions.nii.gz')
    ...
    # Write the CSV summary of affected regions.
    ...
    for label_id in all_label_ids:
        ...
        # Only write rows for labels that are actually affected by stroke and are in the atlas label set.
        if label_id in region_percent_by_label and label_id in atlas_label_id_set:
    ...
    regionAffectPercent = np.array([region_percent_by_label[int(label_id)] for label_id in atlas_label_ids])
    ...
    sc.savemat(os.path.join(affected_regions_dir, affected_regions_prefix + 'labelCount.mat'), label_mat)
...
if __name__ == "__main__":
    ...
    annotation_file = find_single_file(input_folder, '*_AnnoSplit.nii.gz', 'annotation')
