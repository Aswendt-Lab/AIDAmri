...
1) Parental annotation (larger labels):
   ...
   - IMPORTANT: uses the lookup-table keys for filtering IDs (so +2000 R-hemisphere IDs are kept)

2) Standard annotation:
   - Input pattern: **/*_Anno.nii.gz
   - Mask pairing:  _Anno.nii.gz -> _mask.nii.gz  (requested)
...
def compute_region_sizes_from_annotation(
    ...
    Filtering of output IDs:
      - If use_lookup_filter=True: keep only IDs that exist in lookup table keys
        (recommended for par atlas with +2000 hemisphere IDs).
      - Else: keep only IDs that exist in ABALabelIDs loaded from labels_mat_path.
    """
    ...
    if use_lookup_filter:
        # Recommended for par atlas: lookup table already includes both hemispheres (incl. +2000 IDs)
...
def main():
    ...
    # Par atlas lookup (contains +2000 hemisphere IDs, e.g. 22 and 2022)
    lookup_txt_par = os.path.join(base, "lib", "annoVolume+2000_rsfMRI.nii.txt")

    # Regular atlas lookup (if you have a dedicated one, use it; otherwise keep par lookup)
    # You uploaded/mentioned: ARA_annotationR+2000.nii.txt as well; adjust if that is your intended regular lookup.
    lookup_txt_regular = os.path.join(base, "lib", "ARA_annotationR+2000.nii.txt")
    if not os.path.exists(lookup_txt_regular):
        # Fallback to the par lookup if regular lookup isn't present
    ...
    if len(anno_par_paths) > 0:
        out_txt_par, out_mat_par, nreg_par = compute_region_sizes_from_annotation(
            ...
            use_lookup_filter=True,  # <-- critical: keep +2000 hemisphere IDs for par atlas
        )
    ...
    if len(anno_paths) > 0:
        out_txt, out_mat, nreg = compute_region_sizes_from_annotation(
            ...
            mask_replace_to="_mask.nii.gz",  # <-- requested new pairing rule
            use_lookup_filter=False,         # keep old behavior unless you want lookup-driven filtering here too
        )
